...
async def _transcribe_for_teacher(audio_data: bytes, start_ts: float) -> str:
    """Transcribe audio for teacher mode using local or remote ASR."""
    if config.LOCAL_ASR:
        ...
        transcript = await asyncio.to_thread(local_asr.transcribe_wav_bytes, audio_data)
        log.info(_step("VOICE·TEACHER", f"📝 Local transcript: «{transcript[:120]}»", start_ts))
        return transcript

    log.info(_step("VOICE·TEACHER", "📝 LOCAL_ASR disabled, using remote transcription", start_ts))
    transcript = await _transcribe_audio_with_model(audio_data)
    log.info(_step("VOICE·TEACHER", f"📝 Remote transcript: «{transcript[:120]}»", start_ts))
    return transcript
